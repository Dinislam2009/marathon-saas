...
import { 
  Users, BookOpen, ShieldCheck, Layers, Loader2, Star, Clock, Trophy 
} from "lucide-react";
...
export default function AdminStatsPage({ params }) {
      ...
      if (jsonGroups?.ok) {
        ...
        const filteredGroups = (jsonGroups.groups || []).filter(
          (g) =>
            ...
            activeMarathonTitles.has(g.marathon?.title)
        );
  ...
  const curatorsAnalytics = curators.map((m, idx) => {
    ...
    return {
      ...
      name: m.name,
      email: m.email || "—",
      assignedGroup: m.groups?.[0]?.name || (isRu ? "Без группы" : "Топсыз"),
          ...
          <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-xs space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
              ...
              <h2 className="font-extrabold text-slate-900 text-base">
                {isRu ? "Процент заполняемости групп" : "Топтардың Орындау Пайызы"}
              </h2>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {groups.map((group) => {
                  ...
                  const percentage = Math.min(
                    Math.round((studentCount / (group.maxSize || 5)) * 100),
                    100
                  );
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-bold bg-purple-100 text-purple-700 px-2 py-0.5 rounded-lg">
                          {group.marathon?.title || (isRu ? "Марафон" : "Шырақ")}
                        </span>
                      </div>
              ...
              <table className="w-full text-left text-xs font-medium text-slate-600">
                <thead className="bg-slate-50 border-b border-slate-100 text-[10px] uppercase font-black text-slate-400 tracking-wider">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right">{isRu ? "Процент выполнения" : "Орындау Пайызы"}</th>
                  </tr>
                </thead>
