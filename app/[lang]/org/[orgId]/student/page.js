...
export default function StudentDashboardPage() {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      // ⚡ ТҮЗЕТУ: localStorage-тен current_user_id мен currentUser екеуін де тексереміз
      if (typeof window !== "undefined") {
        const savedUserId = localStorage.getItem("current_user_id");
        const userObj = JSON.parse(localStorage.getItem("currentUser") || "{}");
        activeUserId = savedUserId || userObj?.id || null;
      }

      const getDashboardFn = actions.getStudentDashboard;
      let res = null;

      if (typeof getDashboardFn === "function") {
        res = await getDashboardFn(activeStudentId, activeUserId, orgId);
      }
  ...
  }, [currentStudentId, setCurrentStudentId, orgId, isRu]);
  ...
  const handleSubmitReport = async (e) => {
    ...
    try {
      ...
      const submitFn = actions.submitTask;
      let res = null;
