...
import { 
  Plus, Users, BookOpen, Sparkles, Edit3, Trash2, Loader2, X, AlertTriangle, Calendar
} from "lucide-react";
  ...
  const handleSaveEdit = async (e) => {
    ...
    try {
      if (typeof actions.updateMarathon === "function") {
        const res = await actions.updateMarathon(editingMarathon.id, editForm);
        if (res?.ok) {
    ...
    <div className="w-full space-y-6 font-sans text-slate-900 pb-12">
      ...
      {marathons.length === 0 && (
        <div className="bg-white border border-slate-200/80 rounded-3xl p-12 text-center space-y-4">
          ...
          <Link href={`/${lang}/org/${orgId}/admin/marathons/new`} className="inline-block pt-2">
