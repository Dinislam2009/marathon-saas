...
import { 
  ...
  X, Wand2, UserCheck, ChevronRight, Mail, Phone, CheckCircle, Info 
} from "lucide-react";
import { 
  createGroup, deleteGroup, getMarathonsByOrgId, 
  getcuratorsByOrgId, updateGroup, getUnassignedStudents,
  assignStudentToGroup, removeStudentFromGroup, autoDistributeStudents, getGroups 
} from "@/app/actions";
...
export default function OrganizerGroupsPage({ params }) {
  ...
  // ⚡ ШЕКСІЗ ЦИКЛСІЗ ТАЗА ДЕРЕК ТАРТУ (FIXED FETCH DATA)
  const fetchData = useCallback(async (isInitial = false) => {
    ...
    try {
      ...
      const groupsList = await getGroups(orgId);
      ...
      // Драуэр ашық тұрса, оның дерегін цикл тудырмай жаңарту
      setActiveDrawerGroup((prevDrawer) => {
        if (!prevDrawer) return null;
        const fresh = filteredGroups.find((g) => g.id === prevDrawer.id);
        return fresh || prevDrawer;
      });

      setFormData((prev) => {
        if (activeMarathons.length > 0 && !prev.marathonId) {
          return { ...prev, marathonId: activeMarathons[0].id };
        }
        return prev;
      });
    } catch (err) {
  ...
  }, [orgId]); // 👈 Тек orgId өзгергенде ғана орындалады!
  ...
  const handleOpenDrawer = async (group) => {
    ...
    if (group.marathonId) {
      const res = await getUnassignedStudents(group.marathonId);
  ...
  const handleAddStudentToGroup = async () => {
    ...
    const res = await assignStudentToGroup(selectedStudentToAdd, activeDrawerGroup.id);
    if (res?.ok) {
      setSelectedStudentToAdd("");
      showToast(isRu ? "Ученик добавлен в группу" : "Оқушы топқа қосылды", "success");
      await fetchData(false);
      const unassignedRes = await getUnassignedStudents(activeDrawerGroup.marathonId);
      if (unassignedRes?.ok) setUnassignedStudents(unassignedRes.students || []);
    } else {
      showToast((isRu ? "Ошибка: " : "Қате: ") + res?.error, "error");
    }
  ...
  const handleRemoveStudent = async (studentId) => {
    const res = await removeStudentFromGroup(studentId);
    if (res?.ok) {
      showToast(isRu ? "Ученик удален из группы" : "Оқушы топтан шығарылды", "success");
      await fetchData(false);
  ...
  const handleCreateGroup = async (e) => {
    ...
    try {
      ...
      if (res?.ok) {
        ...
        showToast(isRu ? "Группа успешно создана" : "Топ сәтті құрылды", "success");
        await fetchData(false);
      } else {
        showToast((isRu ? "Ошибка: " : "Қате: ") + (res?.error || "Топ құру мүмкін болмады"), "error");
      }
    } catch (err) {
  ...
  const handleUpdateGroup = async (e) => {
    ...
    try {
      setUpdating(true);
      const res = await updateGroup(editFormData.id, editFormData);
      if (res?.ok) {
        setShowEditModal(false);
        showToast(isRu ? "Группа обновлена" : "Топ жаңартылды", "success");
        await fetchData(false);
      } else {
        showToast((isRu ? "Ошибка: " : "Қате: ") + res?.error, "error");
      }
  ...
  const handleAutoDistribute = async () => {
    ...
    if (!targetMarathonId) {
      showToast(isRu ? "Выберите марафон для распределения" : "Авто-бөлу үшін марафонды таңдаңыз", "error");
      return;
    }

    try {
      setAutoDistributing(true);
      const res = await autoDistributeStudents(targetMarathonId);
      if (res?.ok) {
        showToast(
          isRu 
            ? `Ученики успешно распределены! (${res.count || 0} чел.)` 
            : `Оқушылар табысты бөлінді! (${res.count || 0} оқ.)`, 
          "success"
        );
        await fetchData(false);
      } else {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setDeleting(true);
      const res = await deleteGroup(deleteConfirmId);
      if (res?.ok) {
        setDeleteConfirmId(null);
        if (activeDrawerGroup?.id === deleteConfirmId) setActiveDrawerGroup(null);
        showToast(isRu ? "Группа удалена" : "Топ өшірілді", "success");
        await fetchData(false);
      } else {
        showToast((isRu ? "Ошибка: " : "Қате: ") + (res?.error || "Өшіру мүмкін болмады"), "error");
      }
    } catch (err) {
    ...
    <div className="space-y-6 w-full pb-8 font-sans text-slate-900 relative">
      ...
      {toast.show && (
        <div className="fixed top-5 right-5 z-50 animate-in fade-in slide-in-from-top-4 duration-300">
          <div
            className={`flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-xl text-xs font-black border ${
              toast.type === "error"
      ...
      {filteredGroups.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredGroups.map((group) => {
            const studentCount = group.studentsCount || group.students?.length || 0;
            const maxSize = group.maxSize || 30;
            const percentage = Math.min(Math.round((studentCount / maxSize) * 100), 100);
            const isFull = studentCount >= maxSize;
      ...
      {/* 4. ЖАҢА ТОП ҚҰРУ МОДАЛЬДІК ТЕРЕЗЕСЕСІ */}
      {showModal && (
