...
function AddParticipantModal({ onClose }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Осы жерге оқушы қосу логикасын жазасың
    console.log("Жаңа оқушы:", { name, email });
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-sm rounded-2xl p-6 shadow-2xl">
        <h3 className="text-lg font-semibold mb-4">Жаңа қатысушы қосу</h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs text-mist mb-1">Аты-жөні</label>
            <input required value={name} onChange={(e) => setName(e.target.value)} className="w-full border border-mist-light rounded-lg p-2.5 text-sm" placeholder="Иван Иванов" />
          </div>
          <div>
            <label className="block text-xs text-mist mb-1">Email</label>
            <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full border border-mist-light rounded-lg p-2.5 text-sm" placeholder="example@mail.com" />
          </div>
          <div className="flex gap-2 pt-2">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-2 text-sm text-mist hover:bg-paper-dim rounded-lg">Болдырмау</button>
            <button type="submit" className="flex-1 px-4 py-2 text-sm bg-horizon text-white rounded-lg hover:bg-horizon-dark">Қосу</button>
          </div>
        </form>
      </div>
    </div>
...
export default function MentorDashboardPage({ params }) {
  ...
  const [isAddParticipantOpen, setIsAddParticipantOpen] = useState(false); // Жаңа стейт
  ...
  return (
    <div key={tick} className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex justify-between items-center gap-4">
            ...
            <button 
              ...
              className="flex items-center gap-2 bg-horizon text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-horizon-dark transition-colors"
            >
              <Plus size={16} />
              Участник қосу
            </button>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-mist-light text-left text-xs uppercase text-mist tracking-wide">
                  ...
                  <th className="px-5 py-3 font-medium">Тәртід</th>
                </tr>
              </thead>
              <tbody>
      ...
      {isAddParticipantOpen && (
        <AddParticipantModal onClose={() => setIsAddParticipantOpen(false)} />
      )}
    </div>
  );
}