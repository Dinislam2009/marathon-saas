  ...
  return (
    <div key={tick} className="flex flex-col gap-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="font-display text-2xl font-semibold text-ink">Марафондарым</h1>
          <p className="text-mist text-sm mt-1">Барлық марафондарың мен олардың оқушылары осында.</p>
        </div>
        <Link href={`/org/${orgId}/admin/marathons/new`}>
          <Button>
            <Plus size={16} /> Жаңа марафон құру
          </Button>
        </Link>
      </div>
      ...
      {marathons.length === 0 && (
        <Card className="text-center py-12">
          <p className="text-mist">Әзірге марафон жоқ. Біріншісін құрудан баста.</p>
        </Card>
      )}
              ...
              <Card className="h-full flex flex-col gap-3 hover:border-horizon transition-colors">
                ...
                <div className="flex items-center gap-4 text-sm text-mist mt-auto pt-2 border-t border-mist-light">
                  <span className="flex items-center gap-1.5">
                    <Users size={14} /> {students.length} оқушы
                  </span>
                  <span>{tasksCount}/{marathon.durationDays} тапсырма дайын</span>
                </div>
                <p className="text-xs text-mist">Басталды: {formatDate(marathon.startDate)}</p>
                <span className="inline-flex items-center gap-1 text-sm font-medium text-horizon-dark">
                  Басқару <ArrowRight size={14} />
                </span>
              </Card>
            </Link>
          );
