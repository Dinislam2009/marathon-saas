...
export default function SuperAdminPage() {
  const { ready, tick, addOrganizer, setOrganizerSubscriptionStatus } = useData();
  ...
  function handleSubmit(e) {
    ...
    addOrganizer({ ...form, monthlyFee: Number(form.monthlyFee) || 0 });
    setForm(EMPTY_FORM);
    setShowForm(false);
  }

  function toggleBlock(org) {
    const next =
      org.subscriptionStatus === SUBSCRIPTION_STATUS.BLOCKED
        ? SUBSCRIPTION_STATUS.ACTIVE
        : SUBSCRIPTION_STATUS.BLOCKED;
    setOrganizerSubscriptionStatus(org.id, next);
  }
