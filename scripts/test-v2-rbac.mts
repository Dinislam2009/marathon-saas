...
test("RBAC: CURATOR cannot write by default", async () => {
  await assert.rejects(
    ...
    (e: unknown) => e instanceof OrganizationAccessError && e.status === 403,
  );
});
...
test("RBAC: missing authentication is 401", () => {
  assert.throws(
    ...
    (e: unknown) => e instanceof OrganizationAccessError && e.status === 401,
  );
});
...
test("RBAC: explicit write role policy is respected", async () => {
  ...
  await assert.rejects(
    () => requireOrganizationAccess(prismaFor("MANAGER"), "org-1", "user-1", { write: true, writeRoles: ["CURATOR"] }),
    (e: unknown) => e instanceof OrganizationAccessError && e.status === 403,
  );
});
