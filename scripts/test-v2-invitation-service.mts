...
class FakeInvitationRepository implements InvitationRepository {
  ...
  async findPendingByEmail(organizationId: string, email: string) {
    return this.invitations.find((item) => item.organizationId === organizationId && item.email === email && item.status === "PENDING") ?? null;
  }
...
test("invitation service business rules", async () => {
  ...
  await assert.rejects(
    () => service.createInvitation({ organizationId: "org-1", invitedByUserId: "user-1", email: "a@example.com", role: "TEACHER", expiresAt: new Date(Date.now() + 86400000) }, "MANAGER"),
  ...
  await assert.rejects(
    () => service.createInvitation({ organizationId: "org-1", invitedByUserId: "user-1", email: "a@example.com", role: "TEACHER", expiresAt: new Date(Date.now() + 86400000) }, "ADMIN"),
