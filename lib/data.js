...
export async function addOrganizer({ name, ownerName, email, phone, subscriptionPlan, monthlyFee }) {
  ...
  const cleanPhone = (phone || "").trim() || `+7700${Math.floor(1000000 + Math.random() * 9000000)}`;
...
export async function createMarathon(orgId, { title, description, durationDays, startDate }) {
  ...
  if (!validOrgId) {
    throw new Error("Базада бірде-бір Ұйымдастырушы (Organizer) табылмады. Алдымен ұйым құрыңыз.");
  }

  return await prisma.marathon.create({
    data: {
      organizerId: validOrgId,
      title,
...
export async function getcuratorsByOrg(orgId) {
  return await prisma.curator.findMany({ where: { organizerId: orgId } });
}

export async function getcurator(curatorId) {
  return await prisma.curator.findUnique({ where: { id: curatorId } });
}

export async function addcurator(orgId, { name, phone, email }) {
...
export async function getStudentsBycurator(curatorId) {
  return await prisma.student.findMany({ where: { curatorId } });
}
...
export async function addStudentToMarathon(marathonId, { name, email, phone }) {
  try {
    ...
    console.error("❌ Error in addStudentToMarathon:", error);
    throw error;
  }
}

export async function addStudentInvitationBycurator(curatorId, marathonId, { name, email, phone }) {
  const curator = await prisma.curator.findUnique({ where: { id: curatorId } });
  if (!curator) throw new Error("Куратор табылмады");

  await prisma.invitation.create({
    data: {
      marathonId,
      organizerId: curator.organizerId,
...
export async function getTasksByMarathon(marathonId) {
  try {
    ...
    console.error("❌ Error in getTasksByMarathon:", error);
    return [];
  }
}
...
export async function getAllStudentsByOrg(orgId, userId = null) {
  try {
    ...
    console.error("❌ Error in getAllStudentsByOrg:", error);
    return [];
  }
}