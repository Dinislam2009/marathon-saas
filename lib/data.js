...
export function addStudentInvitationByMentor(mentorId, marathonId, { name, email, phone }) {
  const mentor = getMentor(mentorId);
  return addInvitation(marathonId, mentor?.orgId, ROLES.STUDENT, { fullName: name, phone, email });
}
...
export function addStudentInvitationByMentorDuplicate(mentorId, marathonId, { name, email, phone }) {
  const mentor = getMentor(mentorId);
...
export function addStudentToMarathonDuplicate(marathonId, { name, email, phone }) {
  const students = readCollection("students");
  const marathon = getMarathon(marathonId);
