...
describe("ProgramService", () => {
  it("creates and lists programs inside one organization", async () => {
    ...
    const program = await service.createProgram({ organizationId: "org-1", name: "ҰБТ 2027", description: "Exam preparation program", type: "EXAM_PREP" });
  ...
  it("prevents cross-organization reads and updates", async () => {
    ...
    await assert.rejects(() => service.updateProgram("org-b", program.id, { name: "Hacked" }), /Program not found\./);
  });

  it("rejects an invalid date range", async () => {
    const service = new ProgramService(new InMemoryProgramRepository());
    await assert.rejects(() => service.createProgram({ organizationId: "org-1", name: "Invalid", startDate: new Date("2027-05-02"), endDate: new Date("2027-05-01") }), /start date cannot be after end date/i);
  });
});
