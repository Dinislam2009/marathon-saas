...
export function verifyOtp(userId, code) {
  ...
  if (!pending || pending.userId !== userId) {
    return { ok: false, error: "Сессия истекла. Пройдите регистрацию заново." };
  }
  if (Date.now() > pending.expiresAt) {
    return { ok: false, error: "Срок действия кода истёк. Запросите новый код." };
  }
  if (pending.code !== code) {
    return { ok: false, error: "Неверный код. Проверьте ещё раз." };
  }
...
export function loginUser(identifier, password) {
  ...
  if (!user.verified) return { ok: false, error: "Аккаунт ещё не подтверждён." };
