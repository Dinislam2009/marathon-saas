...
/**
 * 1. Ұмытылған нөмірге/email-ге жаңа OTP код генерация жасау
 */
export async function sendResetOtp(identifier) {
  ...
  // Ескі OTP-лерді тазалап, жаңасын жазамыз
  await prisma.pendingOtp.deleteMany({ where: { userId: user.id } });
  await prisma.pendingOtp.create({
    data: {
      userId: user.id,
...
/**
 * 2. Код сәтті тексерілгеннен кейін жаңа құпия сөзді базаға жазу
 */
export async function resetPasswordWithOtp({ userId, code, newPassword }) {
  ...
  // Жаңа құпия сөзді базаға жаңарту (Phase 2-де бұл жерде хэштеу керек)
  await prisma.user.update({
    where: { id: userId },
    data: { 
      passwordHash: newPassword,
      verified: true // Егер бұрын расталмаған болса, біржола расталады
    }
  });

  // Пайдаланылған OTP кодын өшіреміз
  await prisma.pendingOtp.delete({
    where: { userId }
  });
