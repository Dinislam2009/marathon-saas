...
export default function MobileTabBar({ orgId, children }) {
  ...
  const navItems = [
    ...
    { href: `${base}/progress`, label: isRu ? "Календарь" : "Күнтізбе", icon: Calendar },
  ];
  
  const navItemsRight = [
    { href: `${base}/chat`, label: isRu ? "Чат" : "Чат", icon: MessageCircle },
    { href: `${base}/profile`, label: isRu ? "Профиль" : "Профиль", icon: User },
  ];

  const menuItems = [
    { href: `${base}/groups`, label: isRu ? "Группа" : "Топ", desc: isRu ? "Моя команда" : "Менің командам", icon: Users },
    { href: `${base}/materials`, label: isRu ? "Материалы" : "Материалдар", desc: isRu ? "Полезные файлы" : "Пайдалы файлдар", icon: BookOpen },
    { href: `${base}/countdown`, label: isRu ? "Отсчёт" : "Кері санақ", desc: isRu ? "До дедлайна" : "Дедлайнға дейін", icon: Hourglass },
    { href: `${base}/habits`, label: isRu ? "Привычки" : "Әдеттер", desc: isRu ? "Трекер привычек" : "Әдеттер трекері", icon: Target },
    { href: `${base}/leaderboard`, label: isRu ? "Рейтинг" : "Рейтинг", desc: isRu ? "Топ участников" : "Үздік қатысушылар", icon: Trophy },
    { href: `${base}/matrix`, label: isRu ? "Матрица" : "Матрица", desc: isRu ? "Приоритеты Эйзенхауэра" : "Эйзенхауэр басымдықтары", icon: LayoutGrid },
  ];

  const desktopNav = [
    { href: base, label: isRu ? "Главная" : "Басты бет", icon: Home },
    { href: `${base}/progress`, label: isRu ? "Календарь" : "Күнтізбе", icon: Calendar },
    { href: `${base}/groups`, label: isRu ? "Группа" : "Топ", icon: Users },
    { href: `${base}/materials`, label: isRu ? "Материалы" : "Материалдар", icon: BookOpen },
    { href: `${base}/countdown`, label: isRu ? "Отсчёт" : "Кері санақ", icon: Hourglass },
    { href: `${base}/habits`, label: isRu ? "Привычки" : "Әдеттер", icon: Target },
    { href: `${base}/leaderboard`, label: isRu ? "Рейтинг" : "Рейтинг", icon: Trophy },
    { href: `${base}/matrix`, label: isRu ? "Матрица" : "Матрица", icon: LayoutGrid },
    { href: `${base}/chat`, label: isRu ? "Чат" : "Чат", icon: MessageCircle },
    { href: `${base}/profile`, label: isRu ? "Профиль" : "Профиль", icon: User },
  ];

  return (
    <div className="min-h-screen bg-paper flex font-sans text-slate-900">
      <aside className="hidden md:flex w-60 shrink-0 flex-col py-6 px-4 gap-6 bg-paper-dim/60 border-r border-mist-light min-h-screen sticky top-0">
        <div className="px-2">
          ...
          <h1 className="font-display font-semibold text-lg mt-0.5 text-ink">
            {isRu ? "Кабинет ученика" : "Оқушы кабинеті"}
          </h1>
        </div>
      ...
      <div className="md:hidden">
        ...
        >
          <button onClick={() => setOpen(false)} className="w-full flex justify-center py-4 cursor-pointer">
          ...
          <div className="px-6 flex flex-col gap-5">
            <div className="relative">
              ...
              <input
                placeholder={isRu ? "Поиск на платформе..." : "Платформадан іздеу..."}
        ...
        <div className="fixed bottom-6 inset-x-0 px-4 z-50 max-w-lg mx-auto">
          <div className="bg-white/95 backdrop-blur-md py-3 px-4 rounded-[32px] shadow-[0_12px_40px_rgba(124,58,237,0.15)] border border-white/40 flex justify-between items-center">
            ...
            <button
              onClick={() => setOpen((v) => !v)}
              className="flex items-center justify-center w-14 h-14 bg-gradient-to-tr from-horizon to-horizon-dark rounded-full text-white shadow-lg shadow-horizon/30 hover:scale-105 active:scale-95 transition-all -mt-6 shrink-0 cursor-pointer"
            >
