...
export default function FileUpload({ studentId, dayNumber, onUploadSuccess }) {
  ...
  const handleFileChange = async (e) => {
    ...
    // Локальді превью (суретті жүктемей тұрып көрсету)
    ...
    // Server Action арқылы Supabase-ке жіберу
    ...
    if (result.success) {
      ...
      setErrorMsg(result.error || "Файлды жүктеу кезінде қате өтті");
    }
  };

  return (
    <div className="flex flex-col items-center gap-3 p-4 border-2 border-dashed border-purple-200 rounded-xl bg-purple-50/50">
      <input
        type="file"
      ...
      >
        {uploading ? (
          <>
            <span className="animate-spin">⏳</span> Жүктелуде...
          </>
        ) : (
          <>
            <span>📸</span> Тапсырма суретін/файлын таңдау
          </>
        )}
      </label>
